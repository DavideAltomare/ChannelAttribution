...
print("Visit www.channelattribution.net for more information about ChannelAttribution")
...
#DEPRECATED
# def __markov_model_1(vector[string]& vy, vector[unsigned long int]& vc, vector[double]& vv, vector[unsigned long int]& vn , unsigned long int order, unsigned long int nsim,  unsigned long int max_step, unsigned long int out_more, string sep, unsigned long int seed):
	# return(markov_model_cpp(vy,vc,vv,vn,order,nsim,max_step,out_more,sep,seed))
...
def __markov_model_mp_1(vector[string] vy, vector[unsigned long int] vc, vector[double] vv, vector[unsigned long int] vn, unsigned long int order, unsigned long int nsim_start,  unsigned long int max_step, unsigned long int out_more, string sep, unsigned long int ncore, unsigned long int nfold, unsigned long int seed, double conv_par, double rate_step_sim, int verbose):
	return(markov_model_mp_cpp(vy,vc,vv,vn,order,nsim_start,max_step,out_more,sep,ncore,nfold,seed,conv_par,rate_step_sim,verbose))
...
#Data = pd.read_csv('http://localhost:3000/static/csv/Data.csv',sep=";")	
	
"""

**Markov Model Online Multi-Channel Attribution Problem**
...
def heuristic_models(Data,var_path,var_conv,var_value=None, sep=">"):
	...
	>>> Data = pd.read_csv('www.channelattribution.net/static/csv/Data.csv',sep=";")
...
#DEPRECATED
# def markov_model(Data,var_path,var_conv,var_value=None,var_null=None,order=1,nsim=None,max_step=None,out_more=False,sep=">",seed=None,verbose=True):
	...
	# if (verbose == True):
		# warnings.warn("This function is deprecated and it could be removed from future versions. Use markov_model_mp instead. Disable this warning setting vebose=False.")
	
	# res0=__markov_model_1(vy=Data[var_path].str.encode('utf-8'),vc=Data[var_conv],vv=vv,vn=vn,order=order,nsim=nsim,max_step=max_step,out_more=out_more,sep=sep.encode("utf-8"),seed=seed)
...
def markov_model_mp(Data,var_path,var_conv,var_value=None,var_null=None,order=1,nsim_start=1e5,max_step=None,out_more=False,sep=">",ncore=1, nfold=10, seed=0, conv_par=0.05,rate_step_sim=1.5,verbose=True):
	...
	res0=__markov_model_mp_1(vy=Data[var_path].str.encode('utf-8'),vc=Data[var_conv],vv=vv,vn=vn,order=order,nsim_start=nsim_start,max_step=max_step,out_more=out_more,sep=sep.encode("utf-8"),ncore=ncore, nfold=nfold, seed=seed, conv_par=conv_par, rate_step_sim=rate_step_sim, verbose=int(verbose))
...
def auto_markov_model(Data, var_path, var_conv, var_null, var_value=None, max_order=10, roc_npt=100, plot=False, nsim_start=1e5, max_step=None, out_more=False, sep=">", ncore=1, nfold=10, seed=0, conv_par=0.05, rate_step_sim=1.5, verbose=True):
	...
	res = markov_model_mp(Data, var_path, var_conv, var_value = var_value, var_null = var_null, order = best_order, nsim_start = nsim_start, max_step = max_step, out_more = out_more, sep = sep, ncore = ncore, nfold = nfold, seed = seed, conv_par = conv_par, rate_step_sim = rate_step_sim, verbose = verbose)
